...
const { validateLogDataTypes } = require("../../../models/validations");
...
logsV2.get("/", (req,res) => {
    ...
    //HTML (look back at intro to express lab with pokemon example)
    const logsList = logsArray.map((log, index) => {
        return `<li><a href="/v2/logs/${index}">${log.title}</a></li>`
    });
    ...
    const html = `
        <html>
            ...
            <body>
                <h1>Captain Log List</h1>
                <p>Click on a link below to view any log of interest:</p>
                <ul>${logsList.join("")}</>
            </body>
        </html>
    `;
    res.send(html);//remember it is .send instead of .json because we want to send data as a string for HTML display
})

logsV2.get("/:indexOfArray", (req, res) => {
    const index = parseInt(req.params.indexOfArray);
    const selectedLog = logsArray[index];
    const html = `
            <html>
                <head>
                    <title>${selectedLog.title}</title>
                </head>
                <body>
                    <h1>${selectedLog.title}</h1>
                    <p>Captain: ${selectedLog.captainName}</p>
                    <p>Log: ${selectedLog.post}</p>
                    <a href="http://localhost:3333/v2/logs">
                        <button>Go back to view all logs</button>
                    </a>
                </body>
            </html>
    `
...
logsV2.post("/", validateURL, validateLogDataTypes, (req, res) => {
    const newLog = {
